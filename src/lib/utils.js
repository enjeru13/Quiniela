...
export function calcPoints(match, prediction) {
  ...
  const predWinner = pred_home > pred_away ? 'home' : pred_away > pred_home ? 'away' : 'draw'
...
export function computeGroupStandings(matches) {
  ...
  for (const m of matches) {
    ...
    const ensureTeam = (team) => {
      if (!g[team.short]) g[team.short] = { team, pj: 0, g: 0, e: 0, p: 0, gf: 0, gc: 0, pts: 0 }
    }
    ...
    if (m.status === 'finished' && m.home_score !== null && m.away_score !== null) {
      ...
      g[m.home_team.short].pj++; g[m.away_team.short].pj++
      g[m.home_team.short].gf += h; g[m.home_team.short].gc += a
      g[m.away_team.short].gf += a; g[m.away_team.short].gc += h
      if (h > a) {
        g[m.home_team.short].g++; g[m.home_team.short].pts += 3; g[m.away_team.short].p++
      } else if (a > h) {
        g[m.away_team.short].g++; g[m.away_team.short].pts += 3; g[m.home_team.short].p++
      } else {
        g[m.home_team.short].e++; g[m.home_team.short].pts++
        g[m.away_team.short].e++; g[m.away_team.short].pts++
      }
    }
  }
