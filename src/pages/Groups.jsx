    ...
    <div className="bg-ios-card rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-b border-ios-border/30">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-[9px] font-bold text-ios-label3 uppercase tracking-wider">
            Equipo
          </span>
        </div>
        {["PJ", "G", "E", "P", "GF", "GC", "Pts"].map((col) => (
          <span
            key={col}
            className={`text-[9px] font-bold uppercase tracking-wider text-center shrink-0 ${col === "Pts" ? "w-8 text-white/50" : "w-6 text-ios-label3"}`}
          >
            {col}
          </span>
        ))}
      ...
      {group.teams.map((entry, i) => {
        ...
        return (
          ...
          >
            {/* Position bar */}
            <div
              className="w-0.75 h-7 rounded-full shrink-0"
              style={{ background: pos.bar }}
            />
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <TeamFlag
                code={entry.team.flag_code}
                size="sm"
                short={entry.team.short}
              />
              <div className="min-w-0">
                <p
                  className={`text-xs font-bold truncate leading-none ${i < 2 ? "text-white" : "text-ios-label2"}`}
                >
                  {entry.team.name}
                </p>
                {pos.label && (
            ...
            {/* Stats */}
            {[entry.pj, entry.g, entry.e, entry.p, entry.gf, entry.gc].map(
              (val, j) => (
                <span
                  key={j}
                  className="text-xs w-6 text-center tabular-nums text-ios-label2 shrink-0"
                >
                  {val}
                </span>
              ),
            ...
            {/* Points */}
            <span
              className={`text-xs font-black w-8 text-center tabular-nums shrink-0 ${i === 0 ? "text-white" : i < 2 ? "text-ios-label2" : "text-ios-label3"}`}
            >
              {entry.pts}
            </span>
          </motion.div>
      ...
      <div className="mb-5">
        ...
        <div className="flex items-center gap-4 mt-1">
          <p className="text-sm text-ios-label2">
            {groups.length} grupos · 48 equipos
          </p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-3 px-1">
              {[
                ...
                <div key={k} className="flex items-center gap-1.5">
                  <span className="text-[10px] font-black text-ios-label2 w-6 shrink-0">
                    {k}
                  </span>
