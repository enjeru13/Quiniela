...
function TournamentBanner({ played }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden mb-5">
      <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #ff6b35 0%, #ff453a 40%, #1c1c1e 100%)' }} />
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at top right, rgba(255,159,10,0.25) 0%, transparent 60%)' }} />
      <div className="relative p-5">
        <div className="flex justify-between items-start mb-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Trophy size={13} className="text-white/70" />
              <span className="text-[10px] font-black text-white/70 uppercase tracking-[0.15em]">FIFA World Cup</span>
            </div>
            <p className="text-xl font-black tracking-tight leading-tight text-white">USA · CAN · MEX 2026</p>
          </div>
          <div className="text-right rounded-xl px-3 py-1.5" style={{ background: 'rgba(0,0,0,0.25)' }}>
            <p className="text-[10px] font-bold text-white/60 uppercase tracking-wider">Fase</p>
        ...
        <div className="rounded-xl p-3" style={{ background: 'rgba(0,0,0,0.2)' }}>
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-semibold text-white/60 uppercase tracking-wider">Progreso</span>
            <span className="text-[10px] font-bold text-white/80">{played} / {TOTAL_MATCHES}</span>
          </div>
...
function DatePill({ dateStr, isSelected, matchCount, hasLive, onClick }) {
  ...
  const mainLabel = isT ? 'Hoy' : isY ? 'Ayer' : isTom ? 'Mañana' : format(date, 'd', { locale: es })
  const subLabel = hasLabel ? format(date, 'd MMM', { locale: es }) : format(date, 'MMM', { locale: es })
  ...
  return (
    <motion.button
      ...
      className="flex flex-col items-center shrink-0 rounded-2xl relative overflow-hidden"
      style={
        isSelected
          ? {
              background: '#0a84ff',
    ...
    >
      ...
      {isSelected ? (
        <span className="text-[10px] font-semibold mt-1" style={{ color: 'rgba(255,255,255,0.6)' }}>
          {subLabel}
        </span>
      ) : (
        !hasLabel && (
          <span className="text-[9px] font-medium mt-0.5 capitalize" style={{ color: '#636366' }}>
            {subLabel}
          </span>
        )
      )}
      ...
      {dates.map((d) => (
        <div key={d} data-selected={d === selected}>
          <DatePill
            ...
            hasLive={matchesByDate[d]?.some((m) => m.status === 'live') ?? false}
            onClick={() => onSelect(d)}
          />
        </div>
      ))}
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[11px] font-black text-ios-red uppercase tracking-[0.14em]">En Vivo</span>
        <span className="ml-1 text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(255,69,58,0.15)', color: '#ff453a' }}>
          {matches.length}
        </span>
      </div>
...
function TimeSlotHeader({ time, allFinished, allScheduled, count }) {
  if (allFinished) {
    return (
      <div className="flex items-center gap-2 mb-3">
        <CheckCircle2 size={12} style={{ color: '#30d158', opacity: 0.7 }} className="shrink-0" />
        <span className="text-[11px] font-bold text-ios-label3 uppercase tracking-[0.1em]">{time} · Finalizados</span>
  ...
  if (allScheduled) {
    return (
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[11px] font-bold text-ios-label3 uppercase tracking-[0.1em]">{time}</span>
        <span className="ml-auto text-[10px] text-ios-label3">{count}</span>
      </div>
    )
  }
  return (
    <div className="flex items-center gap-2 mb-3">
      <span className="w-2.5 h-[2px] rounded-full shrink-0" style={{ background: '#636366' }} />
      <span className="text-[11px] font-bold text-ios-label3 uppercase tracking-[0.1em]">{time}</span>
      <span className="ml-auto text-[10px] text-ios-label3">{count}</span>
    </div>
  )
...
function LoadingMatches() {
  return (
    <div className="flex flex-col gap-3">
      {[1, 2, 3].map((i) => (
        <div key={i} className="h-40 rounded-2xl animate-pulse" style={{ background: '#1c1c1e' }} />
      ))}
    </div>
  )
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (defaultDate && selectedDate === todayStr() && !matchesByDate[selectedDate]) {
  ...
  return (
    <>
      ...
      {error && (
        <div className="rounded-2xl p-4 mb-5 text-center" style={{ background: '#1c1c1e' }}>
          <p className="text-xs text-ios-red font-semibold">Error cargando partidos</p>
      ...
      {loading ? (
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <LiveSection matches={liveMatches} predictions={predictions} onPredict={setSelectedMatch} />
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="flex flex-col gap-5">
              {timeSlots.map((time) => {
                ...
                const allScheduled = slot.every((m) => m.status === 'scheduled')
                return (
                  <div key={time}>
                    <TimeSlotHeader time={time} allFinished={allFinished} allScheduled={allScheduled} count={slot.length} />
                    <div className="flex flex-col gap-3">
                      {slot.map((match) => (
              ...
              {dayMatches.length === 0 && (
                <p className="text-center text-ios-label3 text-sm py-12">Sin partidos este día</p>
              )}
            </motion.div>
          </motion.div>
        </AnimatePresence>
