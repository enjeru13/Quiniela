...
import { Target, Zap, Crown, Star, Globe, Users } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="relative mb-1.5">
        ...
        <div className="absolute -bottom-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: m.color, boxShadow: "0 1px 5px rgba(0,0,0,0.6)" }}>
      ...
      <div className="w-full rounded-t-xl flex flex-col items-center justify-center gap-1" style={{ height: heights[position], background: m.bg, border: `1px solid ${m.border}`, borderBottom: "none" }}>
...
function ListRow({ player, isLast, isMe }) {
  ...
  return (
    <motion.div variants={item} className={`flex items-center px-4 py-3 gap-3 ${isMe ? "bg-ios-blue/8" : ""} ${!isLast ? "border-b border-ios-border/25" : ""}`}>
      <div className="w-7 flex items-center justify-center shrink-0">
        {player.rank <= 10 ? (
          <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: `${rankColor}1a`, border: `1px solid ${rankColor}40` }}>
  ...
  return (
    <>
      ...
      {me && !meInTop3 && (
        ...
        >
          <div className="rounded-2xl px-4 py-3 flex items-center gap-3" style={{ background: "rgba(10,132,255,0.13)", border: "1px solid rgba(10,132,255,0.3)", backdropFilter: "blur(24px)" }}>
...
export default function Rankings() {
  ...
  // Auto-select league tab if coming from invite/card
  useEffect(() => {
    const ligaId = searchParams.get("liga");
    if (ligaId) { setActiveTab(ligaId); setSearchParams({}); }
  }, []);

  // Fetch league ranking when tab changes
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-ios-label2 mt-1">
            {activeTab === "global" ? `${globalData.length} participantes` : currentLeague?.name}
          </p>
        </div>
        {me && activeTab === "global" && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-2xl shrink-0" style={{ background: "rgba(10,132,255,0.1)", border: "1px solid rgba(10,132,255,0.2)" }}>
      ...
      <AnimatePresence mode="wait">
        {currentLoading ? (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-2.5">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="rounded-2xl animate-pulse" style={{ height: i === 1 ? 180 : 64, background: "#1c1c1e" }} />
            ))}
          </motion.div>
        ) : currentData.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center py-16 gap-3 text-center">
