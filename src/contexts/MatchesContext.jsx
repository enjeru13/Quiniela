...
function transform(m) {
  ...
  return {
    id: m.id,
    home_team: { name: homeInfo.name, short: m.homeTeam?.tla ?? '?', flag_code: homeInfo.flag },
    away_team: { name: awayInfo.name, short: m.awayTeam?.tla ?? '?', flag_code: awayInfo.flag },
...
export function MatchesProvider({ children }) {
  ...
  const fetch = useCallback(() => {
    ...
    getWCMatches()
      .then((raw) => {
        ...
        hasLive.current = transformed.some((m) => m.status === 'live')
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (hasLive.current) fetch()
    }, 60_000)
    return () => clearInterval(id)
  }, [fetch]) // no matches dep — avoids loop

  return (
    <MatchesContext.Provider value={{ matches, loading, error, refetch: fetch }}>
      {children}
